...
public class CCMSSerializer {
	...
	public void run(){
		...
		String fileName = "B1801001";			

		for (Map.Entry<String,SchedulerInfo> entry : Manager.getInstance().getFilesList().entrySet()) {

			SchedulerInfo file = entry.getValue();
			System.out.println(file.getSchInfoName() + " is created!");	
			createSchedulerInfo(true, file.getSchInfoName());			
		}
	}

	public void createSchedulerInfo(boolean flag, String fileName){
		...
		try {
			...
			FileWriter writer = new FileWriter("C:\\CCMSSE\\" + fileName + ".txt");//.SCH
